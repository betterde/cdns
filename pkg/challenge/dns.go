...
func NewChallengeProvider(servers []*dns.Server) ChallengeProvider {
	return ChallengeProvider{servers: servers}
}

// Present is used for making the ACME DNS challenge token available for DNS
func (c *ChallengeProvider) Present(ctx context.Context, chall acme.Challenge) error {
...
func (c *ChallengeProvider) CleanUp(ctx context.Context, challenge acme.Challenge) error {
	for _, s := range c.servers {
		s.PersonalKeyAuth = ""
	}
	return nil
...
func (c *ChallengeProvider) Wait(ctx context.Context, challenge acme.Challenge) error {
	return nil
}
